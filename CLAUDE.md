...
- **`reference/lock-graphs.lua` stays a plain regex-parseable data literal.** The
  Python sims and the Lua `test_solver.lua` parse it; do not let a formatter
  reflow it. The mod prefers a LIVE decode of the game's
  `PrecompiledScript_Shipping.Cache` (`data/livegraphs.lua`, a byte-faithful
  in-process port of `tools/extract_locks.py`). Load order is live decode ->
  self-written cache -> bundled fallback: `data/lockgraphs_fallback.lua` is a
  shipped vanilla graph set (byte-faithful to a verified live decode) used ONLY
  when the live decode and the self-cache both fail (a build/distribution where
  the `.Cache` is unreadable or does not decode). Live is always preferred, so the
  bundle never overrides correct live data and never breaks layout-changing mods
  where the decode works; refresh it from a successful `livegraphs.cache.lua` when
  the vanilla layout changes. `reference/lock-graphs.lua` is the dev-side oracle,
  regenerated by `extract_locks.py`.
