...
- **`reference/lock-graphs.lua` stays a plain regex-parseable data literal.** The
  ...
  reflow it. The shipped mod no longer bundles a dump: it decodes the graph live
  from the game's `PrecompiledScript_Shipping.Cache` (`data/livegraphs.lua`, a
  byte-faithful in-process port of `tools/extract_locks.py`), with a
  self-populating cache fallback. `reference/lock-graphs.lua` is the dev-side
  oracle, regenerated by `extract_locks.py`.
