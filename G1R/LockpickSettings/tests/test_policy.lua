...
--
-- Proves the Lua side of the precompute: util.inflate decodes the shipped DEFLATE
-- blobs (data/lockpolicies.bin), nextmove.policy encodes the live state and looks
-- up the move, and following that move from random reachable states always
...
-- the shipped blob file + a reader (offsets in the index are 0-based)
local bin
do
    local fh = assert(io.open(DIR .. "/../Scripts/data/lockpolicies.bin", "rb"))
    bin = fh:read("*a")
    fh:close()
end
